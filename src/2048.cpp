...
void Game::Render(sf::RenderTarget &tgt) {
  ...
  for (int x = 0; x < FIELD_WIDTH; x++) {
    for (int y = 0; y < FIELD_HEIGHT; y++) {
      tile.setPosition(FIELD_MARGIN + x * (tileSize + TILE_MARGIN),
      ...
      if (map[x][y] != 0) {
        ...
        text.setPosition(FIELD_MARGIN + x * (tileSize + TILE_MARGIN) +
                             (tileSize - fr.width) / 2 - fr.left,
                         FIELD_MARGIN + y * (tileSize + TILE_MARGIN) +
                             (tileSize - fr.height) / 2 - fr.top);
  ...
  for (size_t i = 0; i < moves.size(); i++) {
    ...
    text.setPosition(curPos.x + (tileSize - fr.width) / 2 - fr.left,
                     curPos.y + (tileSize - fr.height) / 2 - fr.top);
